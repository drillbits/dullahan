...
async function run(email, password, fileID, waitMsec, dst, headless) {
  ...
  try {
    const cookie = await hijack.getCookie(browser, page, email, password, waitMsec, fileURL);
    console.debug(cookie);

    const streamMap = await hijack.extractStreamMap(browser, page, cookie, fileURL);
